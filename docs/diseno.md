...
- `Ultimo vivo` existe como variante alternativa de `FFA`, con estructura `first-to` por rondas y sin score por causa; se presenta subordinada a `Score por causa`, que sigue siendo el modo principal.
...
- `setup local` cubre modo, mapa, jugadores y variante de modo. En `FFA` expone `Score por causa` como default y `Ultimo vivo` como variante subordinada; `Teams` conserva `Score por causa`.
