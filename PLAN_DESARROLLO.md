...
- Etapa 8: primera capa de arquetipos ya integrada sobre los sistemas existentes. `RobotArchetypeConfig` combina multiplicadores editables con pasivas chicas reutilizando hooks ya presentes: `Ariete` resiste mas impulso, `Grua` estabiliza otra pieza al devolver partes, `Cizalla` castiga piezas ya dañadas y `Patin` convierte pickups de movilidad en ventanas mas largas; el laboratorio 4P sigue exponiendo ese roster en HUD/marcador sin abrir otra UI ni meter todavia skills exclusivas.
...
- pendiente: decidir via playtest si esta capa de pasivas chicas ya alcanza o si la siguiente diferenciacion debe venir por una skill/regla mas visible para el arquetipo que siga borroso
