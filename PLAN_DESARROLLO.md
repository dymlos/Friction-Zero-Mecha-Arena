...
- Ritmo de apertura reforzado dentro del loop real: `MatchController` ahora inicia cada ronda con un intro corto visible (`Ronda N | arranca en ...`) y no deja avanzar el reloj ni la contraccion hasta que termina; `Main` sincroniza ese estado a `RobotBase`, que bloquea solo movimiento/aim/ataque/skills durante ese beat inicial para acercar el laboratorio al ritmo documentado de `inicio parejo -> analisis -> escalada`.
