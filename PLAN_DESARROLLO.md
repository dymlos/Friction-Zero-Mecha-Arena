...
- Cierre de partida mas legible: cuando el match termina, el HUD ahora suma un `MatchResultPanel` centrado con `Partida cerrada`, ganador, marcador final y `Reinicio | F5 ahora o Xs`; el `RecapPanel` lateral queda como detalle secundario y el laboratorio puede reiniciarse de inmediato sin esperar todo el countdown.
