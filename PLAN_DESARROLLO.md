...
- Roster FFA ahora mas util para oportunismo: `main_ffa.tscn` ya reemplaza el slot de `Grua` por `Aguja` para abrir un primer arquetipo Poke/Skillshot sin romper el laboratorio 2v2 enfocado en rescate aliado.
...
- Etapa 8: ya existe una mezcla mas honesta entre pasivas y primera skill propia. `RobotArchetypeConfig` sigue reutilizando hooks legibles para `Ariete`, `Grua`, `Cizalla` y `Patin`, y ahora tambien puede declarar `core_skill_type/label/cargas/recarga`; `Aguja` abre el primer arquetipo Poke/Skillshot con `Pulso` recargable dentro del laboratorio FFA sin duplicar la escena del robot.
...
- el roster compacto ya usa `Player X / <Arquetipo>` y ahora tambien puede sumar `skill Pulso x/y`, mientras el marcador FFA sigue agregando `[<Arquetipo>]` sin romper la UI actual
- pendiente: decidir via playtest si la combinacion actual (pasivas para `Ariete/Grua/Cizalla/Patin` + skill propia de `Aguja`) ya alcanza o si la siguiente diferenciacion debe venir por el arquetipo faltante de Control/Zona, selector runtime o una economia de ammo mas visible
