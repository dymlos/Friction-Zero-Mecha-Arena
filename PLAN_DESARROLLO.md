...
- El selector runtime ya no se pierde al usar `F6`: `Main` ahora guarda en sesion el slot elegido, los overrides `Easy/Hard` y el `RobotArchetypeConfig` activo por jugador antes del cambio de escena, y los reaplica al boot del siguiente laboratorio para comparar Teams/FFA o base/rapido sin rearmar manualmente el setup.
