...
3. **Validar el nuevo HUD dual y la nueva lectura de daño modular**
   ...
   - medir si `RecapPanel` lateral + `MatchResultPanel` centrado + `Stats | ...` (incluyendo `partes perdidas`) explican suficientemente bien `Decision + Marcador + baja N | causa/autor` sin necesitar otra escena/post-ronda.
