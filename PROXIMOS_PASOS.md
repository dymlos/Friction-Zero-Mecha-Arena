...
2. **Validar el nuevo roster de arquetipos**
   ...
   - medir si las pasivas/skills actuales se entienden por playtest sin otra capa de UI: `Ariete` activando `Embestida` para comprometer choques, `Grua` estabilizando rescates y usando `Iman`, `Cizalla` rematando partes tocadas con el nuevo cue `corte` + pulso corporal y `Patin` activando `Derrape` para reposicionarse sin perder legibilidad.
