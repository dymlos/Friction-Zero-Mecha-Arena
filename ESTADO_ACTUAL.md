...
- primer slice de post-muerte para `Teams`:
  ...
  - el cierre de partida/recap lateral ya tambien acredita ese aporte en `Stats | ...`: el equipo suma `apoyo N (M usos)` segun cuantas cargas tomo y gasto su `PilotSupportShip`, sin abrir otra capa de post-match
...
- cierre FFA mas legible: cuando una ronda/partida libre termina, `MatchController` ahora agrega `Posiciones | 1. ...` al `RecapPanel` y al `MatchResultPanel`, usando score acumulado y el orden real de eliminacion de la ronda final para explicar mejor quien quedo segundo/tercero sin abrir otra pantalla
...
- validacion automatizada del cierre FFA: `ffa_match_result_standings_test.gd` comprueba sobre `main_ffa.tscn` que recap y resultado final publiquen la linea `Posiciones | ...` con winner, runner-up y resto del orden final
...
- Los nuevos arquetipos siguen siendo una capa deliberadamente liviana:
  - hoy combinan tuning + pasivas chicas, una silueta diegetica minima por rol y `Grua/Aguja/Ancla` como primera tanda real de skills propias; todavia no hay selección runtime ni una segunda capa mas profunda de reglas por arquetipo
