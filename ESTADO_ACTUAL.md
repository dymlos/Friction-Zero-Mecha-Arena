...
- el HUD vivo FFA ya no obliga a esperar al recap para entender el ranking: `MatchController.get_round_state_lines()` ahora repite `Posiciones | ...` y `Desempate | ...` durante la ronda activa, reutilizando exactamente el mismo criterio de standings del cierre final para que score y desempate se lean mientras el combate sigue en curso
...
- validacion automatizada de standings vivos FFA: `ffa_live_standings_hud_test.gd` comprueba sobre `main_ffa.tscn` que `get_round_state_lines()` publique `Posiciones | ...` y `Desempate | ...` ya durante la ronda, sin depender solo del recap o del panel final
