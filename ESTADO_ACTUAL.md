...
- primer slice de post-muerte para `Teams`:
  ...
  - esa nave puede recoger pickups `estabilizador` o `energia` ocultos hasta que exista soporte activo; el payload queda visible en el roster como `apoyo estabilizador` / `apoyo energia` y al gastar la accion de utilidad repara la parte activa mas dañada o dispara una `energy surge` corta sobre el aliado vivo
