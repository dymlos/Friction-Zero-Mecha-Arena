extends Area3D
class_name EdgePulsePickup

const RobotBase = preload("res://scripts/robots/robot_base.gd")

signal pickup_collected(robot: RobotBase, item_name: String)

@export var item_name := "pulse_charge"
...
func _try_collect_robot(body: Node) -> bool:
	...
	if not robot.store_carried_item(item_name):
		return false

	_set_available_state(false)
	respawn_timer.start(respawn_delay)
	pickup_collected.emit(robot, item_name)
	return true
