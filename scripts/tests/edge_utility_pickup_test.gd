...
func _validate_main_scene_utility_pickups() -> void:
	for scene_path in MAIN_SCENES:
		...
		_assert(scene is PackedScene, "La prueba utility necesita una PackedScene valida: %s" % scene_path)
		...
		var edge_pickups := _get_edge_utility_pickups(main)
		_assert(edge_pickups.size() >= 2, "La escena %s deberia ofrecer pickups utility en borde." % scene_path)
		...
		_assert(
			pickup_near_edge,
			"Los pickups utility de %s deberian vivir cerca del riesgo de borde, no en el centro limpio." % scene_path
		)
		...
		_assert(active_pickup != null, "La escena %s deberia habilitar utility en al menos un layout del borde." % scene_path)
		...
		if active_pickup != null and scene_robots.size() > 0:
			...
			_assert(
				...
				"El roster compacto deberia dejar visible la estabilidad utility tambien en %s." % scene_path
			)
			_assert(
				status_indicator != null and status_indicator.visible,
				"El robot que asegura utility en %s deberia mostrar un cue diegetico de estabilidad." % scene_path
			)
			var status_message := String(main.ui.status_label.text)
			_assert(
				status_message.contains("estabilidad"),
				"El HUD deberia publicar la adquisicion utility tambien en %s." % scene_path
			)
